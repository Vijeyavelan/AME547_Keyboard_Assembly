...
# Realistic process dwell for vacuum-handled parts.
# Pick dwell = cup seal + vacuum build + pickup confirmation.
...
# Add this to the commanded TCP placement target so the carried part visually reaches
# the final seated pose before release instead of snapping upward at release.
PLACE_TCP_CORRECTION = {
    "s1_epdm":    np.array([-0.0027, -0.0022, 0.0088]),
...
def post_assembly_inspection(m, d, v, placed_locks, timer):
    """Final S1 release-gate inspection.

    This is a simulation representation of the fixed overhead camera over the
    pallet. The dwell gives realistic time for image capture, part-presence
    checks, screw-head verification, and PLC release decision.
    """
    print(); print("="*60); print("[PHASE 10] Post-assembly quality inspection"); print("="*60)
    print("  [QC] Overhead pallet camera active — capturing final assembly image")
    print("  [QC] Checking EPDM, battery, PCB, aluminum plate, and screw-head presence")
    dwell_seconds(m, d, v, POST_ASSEMBLY_INSPECTION_DWELL, locked=placed_locks,
                  label=f"post-assembly camera inspection ({POST_ASSEMBLY_INSPECTION_DWELL:.1f}s)")
    print("  [QC] PASS — S1 assembly verified")
    print("  [QC] Pallet release permitted to downstream buffer")
    timer.mark(d, "Post-assembly inspection")
...
def carry_to(m, d, v, fj, fv, co, ctrl, label, locked=None):
    ...
    while cons < SETTLE_STEPS:
        ...
        # Vacuum-carried parts should remain in their normal world orientation.
        # The plate-specific quaternion belongs only to the internal pin-gripper path.
...
def carried_dwell_seconds(m, d, v, fj, fv, co, seconds=1.0, locked=None,
                          label="carried dwell", quat=None):
    """Hold current robot pose while the selected part remains attached to gripper_tip."""
    sid = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_SITE, "gripper_tip")
    ctrl_hold = d.ctrl.copy()
    q = np.array([1.0, 0.0, 0.0, 0.0]) if quat is None else quat
    steps = max(1, int(seconds / m.opt.timestep))
    for _ in range(steps):
        d.ctrl[:] = ctrl_hold
        if locked:
            for lfj, lfv, pos, lq in locked:
                lock_part(d, lfj, lfv, pos, lq)
        mujoco.mj_forward(m, d)
        tip = d.site_xpos[sid].copy()
        d.qpos[fj:fj+3]   = tip + co
        d.qpos[fj+3:fj+7] = q
...
def print_release_delta(m, d, fj, co, place_pos, bname, label="before release"):
    """Diagnostic only: compare current carried part pose with final seated pose."""
    sid = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_SITE, "gripper_tip")
    mujoco.mj_forward(m, d)
    tip = d.site_xpos[sid].copy()
    carried_pos = tip + co
    actual_qpos = d.qpos[fj:fj+3].copy()
    delta_carried = place_pos - carried_pos
    delta_actual = place_pos - actual_qpos
    print("  -- RELEASE DIAGNOSTIC: " + bname + " (" + label + ")")
    print(f"     tip_world:       {tip.round(6)}")
    print(f"     carry_offset:    {co.round(6)}")
    print(f"     carried=tip+co:  {carried_pos.round(6)}")
    print(f"     actual_part_pos: {actual_qpos.round(6)}")
    print(f"     final_place_pos: {place_pos.round(6)}")
    print(f"     delta final-carried [m]: {delta_carried.round(6)}  | dz={delta_carried[2]*1000:.2f} mm")
    print(f"     delta final-actual  [m]: {delta_actual.round(6)}  | dz={delta_actual[2]*1000:.2f} mm")
...
def drive_screw(m, d, v, name, sfj, sfv, placed_locks=None, home_ctrl=None, screw_box_locks=None):
    ...
    print(f"  [QC] Screw {name} torque/depth OK")
...
def phase_place_plate_internal_lock(m, d, v, fj, fv, pw, plw, waiting, timer):
    ...
    print("  [QC] Pin-gripper release confirmed — aluminum plate seated")
...
def main():
    ...
    with mujoco.viewer.launch_passive(m, d) as v:
        ...
        for step_idx, bname in enumerate(PARTS):
            ...
            dwell_seconds(m, d, v, VACUUM_PICK_DWELL,
                    locked=[(fj_adr, fv_adr, pw, None)] + waiting,
                    label=f"vacuum build / pickup confirm {bname}")
            ...
            for i in range(60):
                ...
                mujoco.mj_step(m, d); v.sync()
            print(f"  ✓ attached")
            print(f"  [QC] Vacuum pressure OK — {bname} picked")
            ...
            tcp_corr = PLACE_TCP_CORRECTION.get(bname, np.zeros(3))
            tip_tgt = plw - co + tcp_corr
            print(f"  place TCP correction for {bname}: {tcp_corr.round(4)} m")
            ...
            # -- PLACE DWELL ------------------------------------------------
            carried_dwell_seconds(m, d, v, fj_adr, fv_adr, co,
                    VACUUM_PLACE_DWELL, locked=waiting,
                    label=f"placement settle / release confirm {bname}")
            ...
            print(f"  [QC] Placement confirmed — {bname}")
